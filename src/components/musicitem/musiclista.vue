    ...
    >
      <div class="musiclist-a">
        ...
        <div class="icon">
          ...
          ></i>
          <i class="iconfont icon-zhengzaibofang" v-if="isplay == 2&&musicid==item.id" @click="changeplay1"></i>
          <i class="iconfont icon-xiai love"></i>
          <i class="iconfont icon-shoucang" @click="addto(item)"></i>
          <i class="iconfont icon-fenxiang"></i>
        </div>
      </div>
      <div class="musiclist-b" v-if="item.ar">
        <span v-for="i in item.ar" :key="i.index">{{ i.name }}</span>
      </div>
      <div class="musiclist-c" v-if="item.al.name">{{ item.al.name }}</div>
      <div class="musiclist-d" v-if="item.dt">
...
export default {
  ...
  methods: {
    chnageplay(id,item) {
      ...
      const findid2=this.playlist.indexOf(item.id)
      if (findid2==-1) {
        const songinformation2=item.id
        this.$store.commit("increment", songinformation2);
      }
    },
    changeplay1(){
